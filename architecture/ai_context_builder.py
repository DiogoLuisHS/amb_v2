...
CODE_EXTENSIONS = {'.ts', '.tsx', '.js', '.jsx', '.mjs', '.cjs', '.py', '.css', '.scss', '.json', '.sql'}
IGNORE_DIRS = {
    'node_modules', 'dist', 'build', '.git', '.antigravity', '.jules', '.amb',
    '.husky', '.vscode', '__pycache__', '.next', '.cache', 'coverage'
}

JS_TS_IMPORT_PATTERNS = [
    re.compile(r"""(?:import|export)\s+(?:[\w\*\$_\s{},]*\s+from\s+)?['"]([^'"]+)['"]"""),
...
class AIContextBuilder:
    ...
    def resolve_import_path(self, current_file: str, import_str: str) -> tuple[str, bool]:
    ...
    def classify_and_order_files(self, file_set: Set[str], query: str = "") -> Dict[str, Any]:
        layers = {
            '1_database_schemas': {'title': '1. Banco de Dados & Schemas (Drizzle / DB Tables)', 'icon': '🗄️', 'files': []},
            '2_repositories': {'title': '2. Repositórios de Acesso a Dados (SQL / Queries)', 'icon': '💾', 'files': []},
            '3_services': {'title': '3. Regras de Negócio & Serviços (Business Logic)', 'icon': '⚙️', 'files': []},
            '4_controllers': {'title': '4. Controladores & Validações de Entrada (Zod / Schemas)', 'icon': '🎮', 'files': []},
            '5_routers_api': {'title': '5. Rotas & Endpoints da API (HTTP / Hono / Express)', 'icon': '🌐', 'files': []},
            '6_frontend_hooks_api': {'title': '6. Frontend Services, Hooks & Chamadas de API', 'icon': '🪝', 'files': []},
            '7_frontend_components': {'title': '7. Componentes de UI & Modais', 'icon': '🧩', 'files': []},
            '8_frontend_pages': {'title': '8. Páginas Web & Views Principais', 'icon': '🖥️', 'files': []},
            '9_entrypoints_config': {'title': '9. Pontos de Entrada & Configurações Globais', 'icon': '🚀', 'files': []},
        }
        ...
        for file in sorted(file_set):
            ...
            if 'db/schema' in file or ('schema' in file and ('apps/api/src/db' in file or 'src/db' in file)):
            ...
            elif ('apps/web' in file or 'web' in file or 'frontend' in file) and ('hooks' in file or 'services' in file or 'api' in file):
                layers['6_frontend_hooks_api']['files'].append(file_meta)
            elif ('apps/web' in file or 'web' in file or 'frontend' in file) and 'components' in file:
                layers['7_frontend_components']['files'].append(file_meta)
            elif ('apps/web' in file or 'web' in file or 'frontend' in file) and ('pages' in file or 'views' in file):
    ...
    def generate_markdown(self, query: str, layers: Dict[str, Any]) -> str:
        ...
        md = []
        md.append(f"# 🧭 ROTEIRO DE LEITURA PARA A IA — MÓDULO / CONTEXTO: `{query.upper()}`\n")
        md.append(f"> **Instrução para a IA:** Para analisar, debugar ou implementar qualquer mudança neste módulo sem se perder no projeto, siga rigorosamente a leitura dos arquivos na ordem arquitetural abaixo (do banco de dados até a interface do usuário):\n")
        md.append(f"**Total de arquivos essenciais:** {total_files} arquivos mapeados\n")
        md.append("---\n")

        for key, layer_info in layers.items():
            if not layer_info['files']:
                continue
            md.append(f"### {layer_info['icon']} {layer_info['title']} ({len(layer_info['files'])} arquivo{'s' if len(layer_info['files']) > 1 else ''})\n")
            for f in layer_info['files']:
                core_badge = " ⭐ **[ARQUIVO PRINCIPAL]**" if f['is_core'] else ""
                md.append(f"- [`{f['path']}`](file:///{f['path']}){core_badge} — *(Consultado por {f['in_count']} | Consulta {f['out_count']})*")
            md.append("")

        md.append("---\n")
        md.append("### 💡 Ordem de Execução & Dependência:")
        md.append("1. **Schema (`db/schema/`):** Define a estrutura das tabelas e os tipos.")
        md.append("2. **Repositories (`repositories/`):** Executam consultas SQL.")
        md.append("3. **Services (`services/`):** Contêm a lógica de negócio.")
        md.append("4. **Controllers & Routers (`controllers/`, `routers/`):** Endpoints HTTP e validações.")
        md.append("5. **Frontend Hooks (`hooks/`):** Gerenciam estado e chamadas de API.")
        md.append("6. **UI Components & Pages (`components/`, `pages/`):** Renderizam as interfaces.")
