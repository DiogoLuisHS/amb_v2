...
def run_monitor(
    interval_seconds: int = 15, check_once: bool = False, auto_approve: bool = False
):
    """Loop principal de monitoramento unificado."""
    print("\n" + "=" * 75)
    mode_text = (
        f"{Colors.BOLD}{Colors.GREEN}[MODO AUTO-APPROVE ATIVO ⚡]{Colors.RESET}"
        if auto_approve
        else "[MODO MONITOR DE ALERTA]"
    )
    print(
        f"{Colors.BOLD}{Colors.CYAN}📡 AMB_V2 — SENTINELA E MONITOR UNIFICADO DE ATENÇÃO (EM TEMPO REAL) {mode_text}{Colors.RESET}"
    )
    print("=" * 75)
    print(f"Intervalo de Polling: {interval_seconds}s")
    print(
        f"Jules API Key:       {'Configurada ✅' if get_env('JULES_API_KEY') else 'Não configurada ⚠️'}"
    )
    print(
        f"Render API Key:      {'Configurada ✅' if get_env('RENDER_API_KEY') else 'Não configurada ⚠️'}"
    )
    print(
        f"Stitch API Key:      {'Configurada ✅' if get_env('STITCH_API_KEY') else 'Não configurada ⚠️'}"
    )
    print(
        f"Gemini/Antigravity:  {'Configurada ✅' if get_env('GEMINI_API_KEY') else 'Não configurada ⚠️'}"
    )
    print("-" * 75)
    print(
        f"{Colors.DIM}Pressione Ctrl+C para encerrar o monitoramento a qualquer momento.{Colors.RESET}\n"
    )
    ...
    while True:
        try:
            ...
            print(
                f"[{now_str}] 🔄 Rodada #{cycle}: Checando chats e status...",
                flush=True,
            )
            ...
            if auto_approve and jules_alerts:
                ...
                for alt in jules_alerts:
                    if alt.get("type") in ["awaiting_feedback", "message"]:
                        ...
                        log(
                            "AUTO-PILOT",
                            f"⚡ Auto-respondendo à sessão {sid} via Antigravity SDK...",
                            Colors.CYAN,
                        )
                        try:
                            ...
                            log_error(
                                "AUTO-PILOT",
                                f"Falha no auto-reply da sessão {sid}: {e}",
                            )
            ...
            if not jules_alerts and not render_alerts:
                notify_info(
                    "Tudo operando normalmente. Nenhuma pendência humana ou erro detectado.\n"
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Monitor unificado de atenção para Jules, Render, Stitch e Antigravity."
    )
    parser.add_argument(
        "--interval",
        "-i",
        type=int,
        default=15,
        help="Intervalo em segundos entre verificações (padrão: 15s).",
    )
    parser.add_argument(
        "--check-once",
        action="store_true",
    ...
    parser.add_argument(
        ...
        "-y",
        action="store_true",
        help="Piloto automático: gera respostas com Antigravity e envia sozinho para todas as dúvidas.",
    )
