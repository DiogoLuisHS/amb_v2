...
def print_audit_report(categorized: Dict[str, List[Dict[str, Any]]]):
    ...
    print("\n" + "=" * 78)
    print(
        f"📊 {Colors.BOLD}{Colors.CYAN}AUDITORIA DE SESSÕES DO GOOGLE JULES — {repo}{Colors.RESET}"
    )
    print("=" * 78)
    print(
        f"Total de sessões encontradas no projeto: {Colors.BOLD}{total}{Colors.RESET}\n"
    )

    print(
        f"✅ {Colors.BOLD}{Colors.GREEN}[1] SESSÕES COM PRs JÁ MERGEADOS NO GIT ({len(categorized['merged'])}) — SEGURAS PARA EXCLUSÃO:{Colors.RESET}"
    )
    for it in categorized["merged"]:
        print(
            f"   • {Colors.GREEN}✔{Colors.RESET} ID: {Colors.DIM}{it['session_id']}{Colors.RESET} | PR: {it['pr_url']} | {it['title'][:55]}"
        )
    ...
    if categorized["completed_no_pr"]:
        ...
        for it in categorized["completed_no_pr"]:
            ...
            print(
                f"   • {Colors.BLUE}ℹ{Colors.RESET} ID: {Colors.DIM}{it['session_id']}{Colors.RESET}{pr_str} | {it['title'][:55]}"
            )
    ...
    if categorized["pending"]:
        ...
        for it in categorized["pending"]:
            print(
                f"   • {Colors.YELLOW}⏳{Colors.RESET} [{it['state']}] ID: {Colors.DIM}{it['session_id']}{Colors.RESET} | {it['title'][:55]}"
            )
    ...
    if categorized["failed"]:
        ...
        for it in categorized["failed"]:
            print(
                f"   • {Colors.RED}✖{Colors.RESET} ID: {Colors.DIM}{it['session_id']}{Colors.RESET} | {it['title'][:55]}"
            )
...
def execute_deletion(
    client: JulesClient, sessions_to_delete: List[Dict[str, Any]], dry_run: bool = False
):
    ...
    print(
        f"{'🔍 [DRY-RUN]' if dry_run else '🗑️ [EXCLUSÃO]'} Processando {total_sessions} sessões..."
    )
    deleted_count = 0
    errors_count = 0
    skipped_count = 0
    ...
    def delete_task(idx, it):
        ...
        if state not in terminal_states:
            print(
                f"   [{idx}/{total_sessions}] {Colors.YELLOW}⏭️ Ignorada (estado '{state}'):{Colors.RESET} ID {sid} - {title}"
            )
            return "SKIPPED", None
        ...
        if dry_run:
            ...
            try:
                ...
                print(
                    f"   [{idx}/{total_sessions}] {Colors.GREEN}✔ Excluída com sucesso:{Colors.RESET} ID {sid} - {title}"
                )
                return "SUCCESS", None
            except Exception as e:
                print(
                    f"   [{idx}/{total_sessions}] {Colors.RED}✖ Falha ao excluir {sid}:{Colors.RESET} {e}"
                )
                return "ERROR", e

    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = [
            executor.submit(delete_task, idx, it)
    ...
    if dry_run:
        print(
            f"🔍 Dry-run concluído: {deleted_count} sessões seriam removidas | {skipped_count} ignoradas."
        )
    else:
        print(
            f"🎉 Limpeza concluída: {deleted_count} sessões excluídas | {errors_count} falhas | {skipped_count} ignoradas."
        )
    print("=" * 78 + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Auditoria e Limpeza de Sessões do Google Jules."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Simula as ações sem executar exclusões reais.",
    )
    parser.add_argument(
        "--delete-merged",
        action="store_true",
        help="Exclui todas as sessões cujos PRs já foram mergeados no Git.",
    )
    parser.add_argument(
        "--delete-all-completed",
        action="store_true",
        help="Exclui todas as sessões concluídas (com e sem PR mergeado).",
    )
    parser.add_argument(
        "--delete-failed",
        action="store_true",
        help="Exclui todas as sessões que falharam (FAILED).",
    )
    ...
    if args.delete_id:
        ...
        if not found:
            try:
                ...
                execute_deletion(
                    client,
                    [{"session_id": args.delete_id, "title": title, "state": state}],
                    dry_run=args.dry_run,
                )
            except Exception as e:
                print(
                    f"{Colors.RED}Erro ao buscar sessão {args.delete_id}:{Colors.RESET} {e}"
                )
        ...
        print(
            f"  • Simular exclusão de PRs integrados: python amb_v2/integrations/jules/tools/cleanup_sessions.py --delete-merged --dry-run"
        )
        print(
            f"  • Excluir PRs já integrados no Git:   python amb_v2/integrations/jules/tools/cleanup_sessions.py --delete-merged"
        )
        print(
            f"  • Excluir todas as sessões concluídas: python amb_v2/integrations/jules/tools/cleanup_sessions.py --delete-all-completed"
        )
        print(
            f"  • Excluir sessões falhas (FAILED):     python amb_v2/integrations/jules/tools/cleanup_sessions.py --delete-failed\n"
        )
