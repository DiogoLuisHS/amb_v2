...
# -*- coding: utf-8 -*-
"""
🧠 AMB_V2 - Executor Dinâmico de Personas Baseado na Pasta .jules/ e Personas (SRP)
Localização: amb_v2/integrations/antigravity/tools/local_agent_runner.py
Responsabilidade Única: Descobrir dinamicamente arquivos markdown de personas na pasta `.jules/` do repositório ativo
...
def get_personas_directory(custom_dir: Optional[str] = None) -> str:
    ...
    candidates = [
        ...
        os.path.join(os.path.dirname(__file__), "..", "..", "agents", "personas"),
...
def list_personas(personas: Dict[str, Dict[str, str]], personas_dir: str):
    ...
    print(f"  • Uma Persona:       amb agent --role <nome>  (ou python local_agent_runner.py --role <nome>)")
...
def main():
    parser = argparse.ArgumentParser(description="Executor dinâmico de personas do repositório ativo (lê dinamicamente da pasta .jules/ ou personas/).")
    ...
    parser.add_argument("--personas-dir", help="Caminho customizado da pasta de personas (padrão: .jules/ na raiz do projeto).")
    ...
    if args.all:
        ...
        print("\n" + "=" * 75)
        print(f"{Colors.BOLD}{Colors.CYAN}🚀 INICIANDO EXECUÇÃO EM LOTE DE TODAS AS {len(personas)} PERSONAS (.jules){Colors.RESET}")
        print("=" * 75)
