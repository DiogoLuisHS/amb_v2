...
class QualityGatekeeper:
    ...
    def run_qa(repo_root: str) -> bool:
        ...
        if not qa_cfg:
            print(
                f"{Colors.GREEN}✔ Nenhuma suíte de QA necessária para este repositório.{Colors.RESET}"
            )
            return True
...
class PipelineOrchestrator:
    ...
    def run(
        ...
        if resume_session:
            log(
                "PIPELINE",
                f"Retomando monitoramento da sessão existente: {resume_session}",
                Colors.HEADER,
            )
        ...
        print("\n" + "=" * 75)
        print(
            f"{Colors.BOLD}{Colors.CYAN}🚀 INICIANDO PIPELINE UNIFICADO AMB_V2: {file_label}{Colors.RESET}"
        )
        print(
            f"📁 Repositório Alvo: {Colors.BOLD}{repo_name}{Colors.RESET} (Branch: {starting_branch})"
        )
        print("=" * 75)

        log(
            "PARSER",
            f"Prompt Visual: {len(stitch_prompt)} chars | Prompt Dev: {len(jules_prompt)} chars",
            Colors.DIM,
        )
        ...
        if sync_ds:
            log(
                "ETAPA 1.1",
                "🎨 Sincronizando Design Tokens locais com o Stitch...",
                Colors.HEADER,
            )
            try:
                sync_design_system()
        ...
        if not skip_stitch:
            log(
                "ETAPA 2/6",
                "🎨 Processando Layout Visual no Stitch SDK...",
                Colors.HEADER,
            )
            if current_screen_id and not edit_screen_id:
                log(
                    "STITCH",
                    f"Buscando tela existente ID: {current_screen_id}...",
                    Colors.CYAN,
                )
                ...
                log(
                    "STITCH",
                    f"Refinando tela ID {edit_screen_id} com novo prompt...",
                    Colors.CYAN,
                )
                screen_data = edit_screen(
                    screen_id=edit_screen_id, prompt=stitch_prompt
                )
            else:
                log(
                    "STITCH",
                ...
                screen_data = generate_screen(
                    prompt=stitch_prompt, device_type=device_type
                )
            ...
            if not auto_approve:
                log(
                    "ETAPA 3/6",
                    "🚪 Gatekeeper 1: Decisão de Design e Layout",
                    Colors.HEADER,
                )
                print("Escolha a próxima ação:")
                print(
                    "  [1] Aprovar mockup e seguir para Síntese Cognitiva & Jules (Recomendado)"
                )
                ...
                try:
                    choice = (
                        input(
                            f"\n{Colors.BOLD}Opção [1-4] (Padrão: 1): {Colors.RESET}"
                        ).strip()
                        or "1"
                    )
                ...
                if choice == "2":
                    instruction = input(
                        f"{Colors.BOLD}Digite a instrução de refinamento: {Colors.RESET}"
                    ).strip()
                    if instruction:
                        log(
                            "STITCH",
                            f"Refinando tela {current_screen_id}...",
                            Colors.CYAN,
                        )
                        screen_data = edit_screen(
                            screen_id=current_screen_id, prompt=instruction
                        )
                        stitch_html = screen_data.get("htmlCode", "")
                        screenshot_url = screen_data.get("screenshotUrl", "")
                        print(
                            f"✅ Refinamento concluído. Nova screenshot: {screenshot_url}"
                        )
                elif choice == "3":
                    log(
                        "STITCH",
                        f"Gerando variantes para a tela {current_screen_id}...",
                        Colors.CYAN,
                    )
                    ...
                    print(
                        f"{Colors.YELLOW}Execução cancelada pelo usuário.{Colors.RESET}"
                    )
                    return
        ...
        log(
            "ETAPA 4/6",
            "🧠 Síntese Cognitiva e Consolidação do Prompt para o Jules...",
            Colors.HEADER,
        )
        ...
        if not skip_stitch and (stitch_html or stitch_prompt):
            log(
                "ANTIGRAVITY",
                "Consolidando e sintetizando especificação visual do Stitch com IA...",
                Colors.CYAN,
            )
        ...
        prompt_sections = [
            ...
            jules_prompt
            if jules_prompt
            else "Implementar funcionalidade conforme padrões do repositório.",
            "",
        ]

        if not skip_stitch and (
            current_screen_id or stitch_summary or screenshot_url or stitch_prompt
        ):
            meta_items = []
            ...
            if screenshot_url:
                meta_items.append(
                    f"- **Screenshot de Referência Visual**: {screenshot_url}"
                )
        ...
        executive_prompt = "\n".join(
            filter(lambda s: s is not None, prompt_sections)
        ).strip()

        log(
            "PIPELINE",
            f"Prompt consolidado para o Jules gerado com sucesso!",
            Colors.GREEN,
        )
        print(f"  📊 {Colors.BOLD}Estatísticas do Prompt Consolidado:{Colors.RESET}")
        print(
            f"     • Prompt de Engenharia (Jules): {len(jules_prompt)} caracteres (100% preservado)"
        )
        if stitch_prompt:
            print(
                f"     • Prompt Visual (Stitch): {len(stitch_prompt)} caracteres (100% preservado)"
            )
        if stitch_summary:
            print(
                f"     • Resumo Arquitetural do Stitch: {len(stitch_summary)} caracteres (ultra-denso)"
            )
        print(
            f"     • Prompt Final para o Jules: {len(executive_prompt)} caracteres (~{len(executive_prompt.split())} palavras)\n"
        )
        ...
        log(
            "ETAPA 5/6",
            "⚡ Despachando Tarefa na Cloud do Google Jules...",
            Colors.HEADER,
        )
        ...
        if not session_id:
            raise AmbError(
                f"Resposta inválida ao criar sessão no Jules: {session_resp}"
            )
        ...
        print(
            f"🔗 Acompanhe em: {Colors.BLUE}https://jules.google.com/sessions/{session_id}{Colors.RESET}"
        )
        print("=" * 75 + "\n")
    ...
    def _monitor_jules_session(
        ...
        final_state = monitor_and_assist_session(
        ...
        if not no_qa:
            log(
                "ETAPA 6/6",
                "🛡️ Gatekeeper 2: Validação Local de Integridade",
                Colors.HEADER,
            )
    ...
    def _parse_single_prompt(markdown_text: str) -> tuple[str, str]:
        ...
        parts = re.split(
            r"(?i)#+\s*(?:1\.\s*)?(?:🎨\s*)?especificação\s+visual", markdown_text
        )
        if len(parts) > 1:
            sub = re.split(
                r"(?i)#+\s*(?:2\.\s*)?(?:⚡\s*)?especificação\s+de\s+engenharia",
                parts[1],
            )
    ...
    @classmethod
    def _extract_clean_rules(
        cls, rules_dir: str, max_chars_per_file: int = 1500
    ) -> str:
        """
    ...
    def _clean_html_for_summary(cls, html: str) -> str:
        ...
        html = re.sub(
            r"<svg\b[^>]*>[\s\S]*?<\/svg>", "[Ícone SVG]", html, flags=re.IGNORECASE
        )
        # Remove base64 data URLs
        html = re.sub(
            r'data:image\/[a-zA-Z]+;base64,[^\s"\']+', "[Imagem Base64]", html
        )
...
"""
        try:
            summary = client_agy.generate_text(
                prompt=prompt_input, system_instruction=system_instruction
            )
            ...
            log(
                "STITCH",
                f"Aviso na síntese de UI via IA ({e}). Usando resumo estruturado de fallback.",
                Colors.YELLOW,
            )
        ...
        if stitch_prompt:
            ...
            fallback_lines.append(
                f"- **Especificação Visual Original**: {fallback_text}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Pipeline Autônomo Design-to-Code (AMB_V2)"
    )
    parser.add_argument(
        "prompt_file", nargs="?", help="Caminho do arquivo markdown de prompt unificado"
    )
    parser.add_argument(
        "--stitch-prompt",
        "-s",
    ...
    parser.add_argument(
        ...
        "-y",
        action="store_true",
        help="Pula confirmações manuais no Gatekeeper 1 de Design",
    )
    parser.add_argument(
        "--skip-stitch",
        action="store_true",
        help="Pula a etapa de layout visual do Stitch e vai direto ao Jules",
    )
    parser.add_argument(
        "--no-qa",
        action="store_true",
        help="Não executa a verificação local de typecheck/build ao final",
    )
    parser.add_argument(
        "--resume-session",
        "-r",
        help="Retoma o monitoramento de uma sessão existente do Jules",
    )
    parser.add_argument(
        "--device-type",
        choices=["DESKTOP", "MOBILE", "TABLET"],
        default="DESKTOP",
        help="Tipo de dispositivo alvo para o Stitch",
    )
    parser.add_argument(
        "--screen-id", help="ID de tela existente no Stitch para reaproveitar"
    )
    parser.add_argument(
        "--edit-screen", help="ID de tela existente para refinar com novo prompt"
    )
    parser.add_argument(
        "--sync-ds",
        action="store_true",
        help="Sincroniza os design tokens locais com o Stitch antes de iniciar",
    )
    parser.add_argument(
        "--branch",
        "-b",
        help="Branch de início para o Jules (Padrão: detecta a atual ou develop)",
    )
    ...
    if (
        ...
        and not args.stitch_prompt
        and not args.jules_prompt
