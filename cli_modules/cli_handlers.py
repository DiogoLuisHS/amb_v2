...
def cmd_prompt(args):
    ...
    if getattr(args, "synthesize", None):
        ...
        prompt_res = synthesize_prompt(
            raw_idea=args.synthesize, role=getattr(args, "role", "general") or "general"
        )
        print("\n" + "=" * 75)
...
def cmd_config(args):
    ...
    if getattr(args, "gemini_confirm", None) is not None:
        enable = args.gemini_confirm.lower() in [
            "on",
            "true",
            "1",
            "yes",
        ...
        status_msg = (
            f"{Colors.GREEN}ATIVADA (Exige confirmação interativa antes de cada requisição ao Gemini){Colors.RESET}"
            if enable
            else f"{Colors.YELLOW}DESATIVADA (Chamadas ao Gemini automáticas){Colors.RESET}"
        )
        print(f"\n🛡️ Autorização prévia do Gemini: {status_msg}\n")
    else:
        status = is_gemini_confirmation_required()
        status_msg = (
            f"{Colors.GREEN}ATIVADA (Exige confirmação manual){Colors.RESET}"
            if status
            else f"{Colors.DIM}DESATIVADA (Chamadas automáticas){Colors.RESET}"
        )
...
def cmd_agent(args):
    ...
    if args.all:
        for idx, (k, p_data) in enumerate(personas.items(), 1):
            ...
            runner.execute_single_persona(
                p_data, task=args.task, dispatch_jules=args.dispatch_jules
            )
        return
    ...
    if clean_role not in personas:
        log_error(
            ...
            hint=f"Disponíveis: {', '.join(personas.keys())}",
        )
        sys.exit(1)

    runner.execute_single_persona(
        personas[clean_role], task=args.task, dispatch_jules=args.dispatch_jules
    )
    ...
    if sub == "list":
        ...
        if getattr(args, "json", False):
            ...
            if isinstance(outputs, list):
                for item in outputs:
                    if isinstance(item, dict) and item.get("pullRequest", {}).get(
                        "url"
                    ):
        ...
        if getattr(args, "message", None):
            ...
            log(
                "JULES",
                f"✅ Mensagem direta enviada para a sessão {args.session_id}!",
                Colors.GREEN,
            )
        ...
        sys.argv = (
            ...
            + (["--auto-latest"] if getattr(args, "auto_latest", False) else [])
        )
        merge_main()
...
def cmd_stitch(args):
    ...
    if sub == "generate":
        ...
        res = client.generate_variants(
            screen_id=args.screen_id, prompt=args.prompt, variant_count=args.count
        )
        log(
            "STITCH",
            f"Variantes geradas com sucesso! ID: {res.get('screenId')}",
            Colors.GREEN,
        )
...
def cmd_render(args):
    ...
    if sub == "status":
        ...
        status_color = (
            Colors.GREEN
            if status == "live"
            else (Colors.RED if "fail" in status.lower() else Colors.YELLOW)
        )
        log(
            "RENDER",
            f"Status do serviço {sid}: {status_color}{status}{Colors.RESET}",
            Colors.CYAN,
        )
        ...
        if deploys:
            ...
            dep_id = dep.get("id")
            log(
                "RENDER-LOGS", f"Logs do Deploy {dep_id} (Serviço: {sid}):", Colors.CYAN
            )
            print(
                f"Painel Live Logs: https://dashboard.render.com/web/{sid}/deploys/{dep_id}"
            )
        else:
        ...
        for item in services:
            ...
            print(
                f"  • ID: {Colors.BOLD}{srv.get('id')}{Colors.RESET} | {Colors.GREEN}{srv.get('name')}{Colors.RESET} ({srv.get('type')})"
            )
