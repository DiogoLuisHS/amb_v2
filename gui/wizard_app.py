...
class DynamicWizard(tk.Tk):
    def __init__(self):
        ...
        # Título
        ttk.Label(
            self, text="⚡ AMB_V2 Command Wizard", font=("Segoe UI", 16, "bold")
    ...
    def _build_runner_tab(self):
        ...
        ttk.Label(proj_badge, text="Projeto:", font=("Segoe UI", 9, "bold")).pack(
            side="left"
        )
        ttk.Label(
            proj_badge,
        ...
        ttk.Label(proj_badge, text="Diretório:", font=("Segoe UI", 9, "bold")).pack(
            side="left"
        )
        ttk.Label(
            proj_badge,
        ...
        ttk.Label(
            frame_cmd, text="1. Selecione o Comando:", font=("Segoe UI", 10, "bold")
        ).pack(side="left")

        self.selected_cmd = tk.StringVar()
        cmd_choices = list(self.command_map.keys())
        self.cb_cmd = ttk.Combobox(
            frame_cmd,
            textvariable=self.selected_cmd,
            values=cmd_choices,
            state="readonly",
            width=40,
        )
        ...
        self.cmd_desc_var = tk.StringVar(
            value="Selecione um comando para ver os detalhes."
        )
        lbl_desc = ttk.Label(
            container, textvariable=self.cmd_desc_var, foreground="gray", wraplength=700
        )
        ...
        ttk.Label(
            container, text="2. Parâmetros Disponíveis:", font=("Segoe UI", 10, "bold")
        ...
        ttk.Button(
            frame_btn, text="Executar no Terminal", command=self.execute_cmd
        ).pack(side="right")
    ...
    def on_command_select(self, event):
        ...
        self.cmd_desc_var.set(
            help_text if help_text else f"Executa a operação '{cmd_name}'."
        )
    ...
    def _render_action(self, action):
        ...
        if is_flag:
            ...
            self.param_vars[action.dest] = {
                "type": "flag",
                "var": var,
                "opts": action.option_strings,
            }
            ttk.Checkbutton(
                frame, text=f"{name_label} ({help_text})", variable=var
            ).pack(anchor="w")

        elif is_positional or is_required:
            ttk.Label(
                frame,
                text=f"{name_label} [Obrigatório]: ",
                font=("Segoe UI", 9, "bold"),
            ).pack(side="left")
            ...
            ttk.Label(frame, text=f"  ℹ {help_text}", foreground="gray").pack(
                side="left"
            )
            ...
            if (
                "arquivo" in help_text.lower()
                or "caminho" in help_text.lower()
                or "file" in help_text.lower()
                or "prompt" in help_text.lower()
            ):
                btn = ttk.Button(
                    frame,
                    text="Browse...",
                    state="disabled",
                    command=lambda v=val_var: self._browse(v),
                )
                ...
                def toggle(u=use_var, i=inp, b=btn):
                    ...
                    st_cb = (
                        "readonly" if u.get() and isinstance(i, ttk.Combobox) else st
                    )
            ...
            ttk.Label(
                frame, text=f"  ℹ {help_text}", foreground="gray", wraplength=400
            ).pack(side="left")
    ...
    def _build_settings_tab(self):
        ...
        header_frame = ttk.LabelFrame(
            container, text="Contexto do Projeto Ativo", padding=10
        )
        ...
        ttk.Label(
            row_top, text="Repositório:", font=("Segoe UI", 9, "bold"), width=15
        ).pack(side="left")
        ttk.Label(
            row_top,
            textvariable=self.repo_name_var,
        ...
        ttk.Label(
            row_dir, text="Diretório:", font=("Segoe UI", 9, "bold"), width=15
        ).pack(side="left")
        ttk.Label(row_dir, textvariable=self.project_root_var, wraplength=600).pack(
            side="left"
        )
        ...
        ttk.Label(
            row_env, text="Arquivo .env:", font=("Segoe UI", 9, "bold"), width=15
        ).pack(side="left")
        ttk.Label(
            row_env,
            textvariable=self.env_path_var,
            foreground="#555555",
            wraplength=600,
        ).pack(side="left")
        ...
        ttk.Button(
            btn_bar, text="➕ Adicionar Variável", command=self._add_custom_env_var
        ).pack(side="left")
        ttk.Button(btn_bar, text="🔄 Recarregar", command=self._load_env_fields).pack(
            side="left", padx=5
        )
        ttk.Button(
            btn_bar, text="💾 Salvar Configurações no .env", command=self.save_env
        ).pack(side="right")

    def _choose_project_dir(self):
        chosen = filedialog.askdirectory(
            initialdir=self.project_root, title="Selecione a Pasta do Projeto"
        )
        if chosen:
    ...
    def _render_env_row(self, key, val):
        ...
        lbl = ttk.Label(
            row, text=key, width=25, font=("Segoe UI", 9, "bold"), anchor="w"
        )
        lbl.pack(side="left")
    ...
    def _add_custom_env_var(self):
        var_name = simpledialog.askstring(
            "Nova Variável",
            "Nome da variável de ambiente (ex: MINHA_CHAVE):",
            parent=self,
        )
        if var_name:
            ...
            if var_name in self.env_entries:
                messagebox.showwarning(
                    "Aviso", f"A variável '{var_name}' já existe na lista!", parent=self
                )
            else:
    ...
    def save_env(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Erro ao Salvar", f"Não foi possível gravar no arquivo .env:\n{e}"
            )
